...
//测量相关
 ...
 Matrix<double, 3, 3> UPSData::O_DM_r = MatrixXd::Zero(3, 3);	//测量坐标系下 动平台坐标系原点关系:旋转矩阵(平台标定时用，其余时候不用)
 Matrix<double, 3, 1> UPSData::O_DM_t = MatrixXd::Zero(3, 1);	//测量坐标系下 动平台坐标系原点坐标:平移矩阵(平台标定时用，其余时候不用)
 Matrix<double, 3, 3> UPSData::O_SM_r = MatrixXd::Zero(3, 3);	//测量坐标系下 静平台坐标系原点关系:旋转矩阵(平台标定时用，其余时候不用)
 Matrix<double, 3, 1> UPSData::O_SM_t = MatrixXd::Zero(3, 1);	//测量坐标系下 静平台坐标系原点坐标:平移矩阵(平台标定时用，其余时候不用)
